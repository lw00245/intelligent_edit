...
export function FrameImageViewer({
  images,
  initialIndex,
  onClose,
}: {
  ...
  return createPortal(
    ...
    >
      ...
      <div
        style={{ position: "absolute", bottom: 24, display: "flex", gap: 16, alignItems: "center" }}
        onClick={(e) => e.stopPropagation()}
      >
        <button
          onClick={prev}
          style={{ background: "rgba(255,255,255,0.35)", border: "2px solid rgba(255,255,255,0.7)", borderRadius: 8, padding: "10px 28px", color: "#fff", fontSize: 15, fontWeight: 600, cursor: "pointer", textShadow: "0 1px 3px rgba(0,0,0,0.5)" }}
        >
          &#9664; 上一张
        </button>
...
export function FrameDataPreview() {
  const [images, setImages] = useState<FrameImage[]>([]);
  const [viewerIdx, setViewerIdx] = useState<number | null>(null);

  useEffect(() => {
    fetch("/api/quantilized-images")
      .then((r) => r.json())
      .then((data) => {
        setImages(data.images.map((name: string) => ({ name, url: `/api/quantilized-image?name=${encodeURIComponent(name)}` })));
      })
      .catch(() => {});
  }, []);
  ...
  return (
    <>
      <div className="table-panel" style={{ boxShadow: "none", background: "rgba(255,255,255,0.54)" }}>
        <div className="table-header" style={{ gridTemplateColumns: "100px 1.2fr 0.8fr 1fr" }}>
          <div>缩略图</div>
          <div>帧名称</div>
          <div>状态</div>
        ...
        <div className="table-body">
          {images.map((img, i) => (
            <div key={img.name} className="table-row" style={{ gridTemplateColumns: "100px 1.2fr 0.8fr 1fr" }}>
              <div style={{ width: 80, height: 50, borderRadius: 4, overflow: "hidden", background: "#eee" }}>
                <img src={img.url} alt="" style={{ width: "100%", height: "100%", objectFit: "cover" }} />
              </div>
              <div className="stack">
                <strong>{img.name}</strong>
                <span className="muted">量化帧数据</span>
              </div>
              <div>
                <span className="badge success">已完成</span>
              </div>
              <div className="table-actions">
                <button className="text-button" type="button" onClick={() => setViewerIdx(i)}>
      ...
      {viewerIdx !== null && (
        <FrameImageViewer images={images} initialIndex={viewerIdx} onClose={() => setViewerIdx(null)} />
      )}
    </>
  );
}
